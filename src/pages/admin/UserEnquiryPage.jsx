...
export default function AdminEnquiries() {
  ...
  // ── Row click: open drawer with selected detail ──
  const handleRowClick = async (enquiry) => {
    try {
      const res = await getEnquiryById(enquiry._id);
      setSelectedDetail(res.data);
    } catch (err) {
      showToast('Failed to load enquiry details');
    }
  };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      showToast(err.response?.data?.error || err.message || 'Failed to delete enquiry');
    } finally {
  ...
  return (
    <div style={{ fontFamily: 'Nunito, sans-serif' }}>
      ...
      {selectedDetail && (
        <EnquiryDrawer
          enquiry={selectedDetail}
          onClose={() => setSelectedDetail(null)}
          onStatusChange={handleStatusChange}
          onUpdate={handleUpdate}
          onDeleteRequest={(id, label) => setConfirmDelete({ id, label })}
          onConvert={handleConvertOpen}
        />
      )}
      ...
      {statsLoading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '12px', marginBottom: '16px' }}>
          {[
            { label: 'Today',        value: stats.todayEnquiries ?? 0,       icon: '📅', days: 0 },
            { label: 'Last 7 Days',  value: stats.thisWeekEnquiries ?? 0,    icon: '🗓️', days: 7 },
            { label: 'Last 1 Month', value: stats.thisMonthEnquiries ?? 0,   icon: '📆', days: 30 },
            { label: 'Last 1 Year',  value: stats.lastYearEnquiries ?? stats.thisYearEnquiries ?? stats.total ?? 0, icon: '📈', days: 365 },
          ].map(c => (
            <div 
              key={c.label} 
              onClick={() => {
                const to = new Date();
                const from = new Date();
                from.setDate(from.getDate() - c.days);
                setDateTo(to.toISOString().split('T')[0]);
                setDateFrom(from.toISOString().split('T')[0]);
                setPage(1);
              }}
              style={{ background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', padding: '14px 18px', display: 'flex', alignItems: 'center', gap: '12px', cursor: 'pointer', transition: 'transform 0.15s, box-shadow 0.15s' }}
              onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 4px 12px rgba(0,0,0,0.1)'; }}
              onMouseLeave={e => { e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = ''; }}
            >
