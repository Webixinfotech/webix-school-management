...
import {
  ...
  ChevronDown, CheckCircle, ArrowLeft, Shield, Calendar, FileText, Clock, Hourglass, Lock,
} from 'lucide-react';
...
const FieldErr = ({ k, errors }) =>
  errors[k] ? (
    <p style={{ margin: '4px 0 0', fontSize: 11, color: '#E11D48', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4 }}>
      <AlertCircle size={10} />{errors[k]}
    </p>
  ) : null;

const SectionCard = ({ title, icon: Icon, accent = '#2563EB', children, extra }) => (
  <div className="section-card" style={{ background: '#fff', borderRadius: 12, border: '1.5px solid #E9ECF0', overflow: 'hidden' }}>
    <div style={{ padding: '12px 16px', borderBottom: '1.5px solid #F1F5F9', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#FAFBFC', flexWrap: 'wrap', gap: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 800, color: '#1E293B', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{title}</span>
      </div>
...
const Field = ({ label, required, children, span }) => (
  <div style={span === 'full' ? { gridColumn: '1/-1' } : {}}>
    {label && <label style={LS}>{label}{required && <span style={{ color: '#E11D48', marginLeft: 2 }}>*</span>}</label>}
    {children}
  </div>
);
              ...
              {cls.classType === 'FIXED_TIME' && (
                <div style={{ marginTop: 9, padding: '9px 11px', background: '#fff', borderRadius: 8, border: `1.5px solid ${T.blue.border}`, display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <div>
                    ...
                    <p style={{ margin: '2px 0 0', fontSize: 12, fontWeight: 800, color: '#0F172A' }}>
              ...
              {cls.classType === 'FLEX_TIME' && (
                <div style={{ marginTop: 9 }}>
                  ...
                  <div style={{ marginTop: 8, padding: '9px 11px', background: '#fff', borderRadius: 8, border: `1.5px solid ${T.teal.border}` }}>
                    <label style={LS}>Hours to Assign <span style={{ color: '#E11D48' }}>*</span></label>
                    <input
...
const Toast = ({ message, type = 'error', onClose }) => {
  ...
  const s = {
    ...
    info:    { bg: '#EFF6FF', border: '#BFDBFE', text: '#1D4ED8', icon: 'i' },
  }[type] || {};
    ...
    (async () => {
      ...
      if (!exists) {
        setToast({ message: `Student with ID "${id}" not found. Redirecting...`, type: 'error' });
            setTimeout(() => navigate('/teacher/manage-students'), 2000);
        return;
      }
      try {
        setLoading(true);
        ...
        setForm({
          ...
          photo: null,
          documentVerification: s.documentVerification || {},
        });
        ...
        setTimeout(() => navigate('/teacher/manage-students'), 1500);
      } finally {
        setLoading(false);
      }
    })();
    ...
    try {
      if (isEditMode) {
        ...
        if (form.parentPassword && form.parentPassword.length >= 6 && parentUserIdState) {
          try {
            const pwResponse = await fetch(`${API_BASE_URL}/auth/teacher/change-parent-password/${parentUserIdState}`, {
              method: 'PUT',
              headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        ...
        if (form.photo && typeof form.photo !== 'string') {
          try {
            ...
            setTimeout(() => navigate('/teacher/manage-students'), 3000);
            return;
          }
        }

        if (passwordUpdateFailed) {
          setToast({ message: `${form.name} updated, but the parent password could not be changed. Please ask admin to reset it.`, type: 'warning' });
        } else if (feeSync.failed.length > 0) {
          setToast({ message: `${form.name} updated, but fee enrollment failed for ${feeSync.failed.map(f => f.name).join(', ')}. Please ask admin to enroll from Fee Hub.`, type: 'warning' });
        ...
        if (createdStudentId && (form.classIds || []).length > 0) {
          ...
          if (failed.length > 0) {
            feeEnrollError = "Student created successfully, but class enrollment requires Fee Hub access — ask an admin to enroll this student in a class.";
          }
        } else {
        ...
        if (feeEnrollError) {
          setToast({ message: feeEnrollError, type: 'warning' });
        }
      }
    } catch (e) {
    ...
    <div style={{ minHeight: '100vh', background: '#F1F5F9', fontFamily: "'DM Sans', 'Segoe UI', sans-serif" }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .topbar-title h1 { margin: 0; font-size: 15px; font-weight: 800; color: #0F172A; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        ...
        .cred-value { font-size: 12px; font-weight: 700; color: #0F172A; font-family: monospace; flex: 1; word-break: break-all; }
        ...
        .btn-done { width: 100%; padding: 12px; border-radius: 12px; border: none; background: linear-gradient(135deg,#0F172A,#1E293B); color: #fff; font-weight: 700; font-size: 14px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; }
      ...
      {hasPermission === false && (
        <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#F1F5F9' }}>
          <div style={{ textAlign: 'center', maxWidth: 400, padding: 32, background: '#fff', borderRadius: 16, border: '1.5px solid #E9ECF0' }}>
            ...
            <p style={{ fontSize: 13, color: '#64748B', marginBottom: 20 }}>You don't have permission to register students. Contact an administrator.</p>
      ...
      {hasPermission === null || loading ? (
        <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ textAlign: 'center' }}>
            ...
            <p style={{ marginTop: 16, fontSize: 14, color: '#64748B', fontWeight: 600 }}>Loading...</p>
          </div>
        </div>
      ) : ( 
        <>
          ...
          <div style={{ background: '#fff', borderBottom: '1px solid #E2E8F0', position: 'sticky', top: 0, zIndex: 100 }}>
            <div className="topbar-inner">
              ...
              <div className="topbar-actions">
                <button className="btn-cancel-top" onClick={() => navigate('/teacher/manage-students')}>Cancel</button>
                <button
                  className="btn-save-top"
                  onClick={handleSave}
                  disabled={saveLoading}
                  style={{ background: saveLoading ? '#93C5FD' : '#2563EB', cursor: saveLoading ? 'not-allowed' : 'pointer', boxShadow: saveLoading ? 'none' : '0 2px 8px rgba(37,99,235,0.3)' }}
                >
                  {saveLoading
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div className="two-col">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  {/* Student Details */}
                  <SectionCard title="Student Details" icon={GraduationCap} accent="#2563EB"
                    extra={
                      <div className="photo-area">
                        {photoPreview
                          ...
                          : form.name
                            ? <div onClick={() => fileRef.current?.click()} style={{ width: 36, height: 36, borderRadius: 8, background: 'linear-gradient(135deg,#1E88E5,#1565C0)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 16, cursor: 'pointer', flexShrink: 0 }}>{form.name.charAt(0).toUpperCase()}</div>
                        ...
                        {isEditMode && studentId && (
                          <div className="student-id-badge">
                            ...
                            <span style={{ fontSize: 11, color: '#1E293B', fontWeight: 800, fontFamily: 'monospace' }}>{studentId}</span>
                          </div>
                        )}
                      </div>
                    }
                  ...
                  {/* Father's Info */}
                  <SectionCard title="Father's Info & Parent Login" icon={User} accent="#2563EB">
                    <div className="form-grid">
                      <Field label="Father's Name">
                        <input value={form.fatherName} onChange={e => set('fatherName', e.target.value)} placeholder="Father's full name" style={{ ...IS, borderColor: errors.fatherName ? '#E11D48' : '#E2E8F0' }} />
                        <FieldErr k="fatherName" errors={errors} />
                      </Field>
                      <Field label="Mobile Number" required>
                        <input value={form.fatherMobile} onChange={e => { const v = e.target.value.replace(/\D/g, ''); if (v.length <= 10) set('fatherMobile', v); }} placeholder="10-digit mobile" maxLength={10} style={{ ...IS, borderColor: errors.fatherMobile ? '#E11D48' : '#E2E8F0' }} />
                      ...
                      <Field label="Email Address" required>
                        <input type="email" value={form.fatherEmail} onChange={e => set('fatherEmail', e.target.value)} placeholder="Used for parent login" style={{ ...IS, borderColor: errors.fatherEmail ? '#E11D48' : '#E2E8F0' }} />
                    ...
                    <div style={{ marginTop: 12, background: '#EFF6FF', borderRadius: 9, padding: '12px 13px', border: '1.5px solid #BFDBFE' }}>
                      <label style={{ ...LS, color: '#1D4ED8', marginBottom: 6 }}>
                        Parent Login Password {!isEditMode && <span style={{ color: '#E11D48' }}>*</span>}
                      </label>
                      <input type="text" value={form.parentPassword || ''} onChange={e => set('parentPassword', e.target.value)} placeholder={isEditMode ? 'Leave blank to keep current' : 'Min 6 characters'} style={{ ...IS, fontFamily: 'monospace', borderColor: errors.parentPassword ? '#E11D48' : '#BFDBFE', background: '#fff' }} />
                  ...
                  <SectionCard title="Mother's Info" icon={Heart} accent="#9333EA">
                    <div className="form-grid">
                      <Field label="Mother's Name">
                        <input value={form.motherName} onChange={e => set('motherName', e.target.value)} placeholder="Mother's full name" style={{ ...IS, borderColor: errors.motherName ? '#E11D48' : '#E2E8F0' }} />
                        <FieldErr k="motherName" errors={errors} />
                      </Field>
                      <Field label="Mobile Number">
                        <input value={form.motherMobile} onChange={e => { const v = e.target.value.replace(/\D/g, ''); if (v.length <= 10) set('motherMobile', v); }} placeholder="10-digit mobile" maxLength={10} style={{ ...IS, borderColor: errors.motherMobile ? '#E11D48' : '#E2E8F0' }} />
                      ...
                      <Field label="Email Address">
                        <input type="email" value={form.motherEmail} onChange={e => set('motherEmail', e.target.value)} placeholder="Optional" style={{ ...IS, borderColor: errors.motherEmail ? '#E11D48' : '#E2E8F0' }} />
                ...
                <div className="bottom-bar-btns">
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background: saveLoading ? '#93C5FD' : '#2563EB',
            ...
            <div className="modal-body">
              ...
              {feeEnrollSummary && (feeEnrollSummary.enrolled.length > 0 || feeEnrollSummary.failed.length > 0) && (
                <div style={{ marginTop: 10, background: feeEnrollSummary.failed.length > 0 ? '#FFFBEB' : '#EFF6FF', border: `1.5px solid ${feeEnrollSummary.failed.length > 0 ? '#FDE68A' : '#BFDBFE'}`, borderRadius: 12, padding: '12px 14px' }}>
                  <p style={{ margin: 0, fontSize: 11, color: feeEnrollSummary.failed.length > 0 ? '#92400E' : '#1D4ED8', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  <button
                    ...
                    style={{
                      ...
                      minWidth: 120,
                      borderRadius: 10,
                      border: selectedRecipient === 'Father' ? '2px solid #2563EB' : '1.5px solid #CBD5E1',
                      background: selectedRecipient === 'Father' ? '#EFF6FF' : '#fff',
                      color: selectedRecipient === 'Father' ? '#1D4ED8' : '#475569',
                      padding: '10px 12px',
                      fontSize: 12,
                      fontWeight: 700,
                      cursor: 'pointer',
              ...
              <div className="cred-box" style={{ background: 'linear-gradient(135deg,#EFF6FF,#DBEAFE)', border: '1.5px solid #BFDBFE' }}>
                <p style={{ margin: 0, fontSize: 11, fontWeight: 700, color: '#1D4ED8', marginBottom: 4 }}>Parent Credentials</p>
                <div className="cred-row">
                  <span className="cred-label" style={{ color: '#1D4ED8' }}>Email:</span>
                  <span className="cred-value">{createdCredentials.email}</span>
                  <button className="btn-copy" style={{ color: '#1D4ED8' }} onClick={() => navigator.clipboard.writeText(createdCredentials.email)}>Copy</button>
                </div>
                {createdCredentials.fatherPhone && (
                  <div className="cred-row">
                    <span className="cred-label" style={{ color: '#1D4ED8' }}>Phone:</span>
                    <span className="cred-value">{createdCredentials.fatherPhone}</span>
                  </div>
                )}
                <div className="cred-row">
                  <span className="cred-label" style={{ color: '#1D4ED8' }}>Password:</span>
              ...
              <button className="btn-done" onClick={() => { setShowCredentials(false); setCreatedCredentials(null); navigate('/teacher/manage-students'); }}>
