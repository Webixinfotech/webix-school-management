...
import {
  UserPlus, X, Eye, Edit2, Trash2, QrCode, Clock, BookOpen,
  ...
  Share2, MessageCircle, Smartphone, Loader2, Zap, BarChart3,
} from 'lucide-react';
...
// ─── Student Card ─────────────────────────────────────────────────────────────
const StudentCard = ({ student, onView, onEdit, onDelete, onQR, onToggleStatus, onFlexReport, classesData = [] }) => {
...
const AdminStudentsPage = () => {
  ...
  // Fetch academic sessions for the session filter dropdown
  useEffect(() => {
    (async () => {
      try {
  ...
  }, [pagination.page, pagination.limit, search, statusFilter, sessionFilter, classFilter, classesData]);
  ...
  }, [search, classFilter, statusFilter, sessionFilter]);
  ...
  const hasActiveFilters = search || searchInput || classFilter !== 'all' || statusFilter !== 'all' || sessionFilter !== 'all' || statsFilter;

  const open = (type, s = null) => {
    if (type === 'edit' && s) {
      const studentIdForUrl = s.enrollmentId || s._id;
      navigate(`/admin/students/edit/${studentIdForUrl}`);
      return;
    }
    if (type === 'view' && s) {
      const studentIdForUrl = s.admissionNo || s._id;
      navigate(`/admin/students/${studentIdForUrl}`);
      return;
    }
    setSelected(s);
    setModal(type);
    ...
    try {
      ...
      if (selected?._id) {
        ...
        if (feeSync.failed.length > 0) {
          setToast({ message: `Student updated, but fee enrollment failed for ${feeSync.failed.map(f => f.name).join(', ')}. Please enroll from Fee Hub.`, type: 'warning' });
        ...
        if (pa) {
          ...
          if (feeSync.failed.length > 0) {
            setToast({ message: `Student created, but fee enrollment failed for ${feeSync.failed.map(f => f.name).join(', ')}. Please enroll from Fee Hub.`, type: 'warning' });
          } else if (feeSync.enrolled.length > 0) {
  ...
  const executeDelete = async () => {
    ...
    try { 
      await deleteStudentAPI(confirmDelete._id); 
      setStudentData(p => p.filter(x => x._id !== confirmDelete._id && x.id !== confirmDelete._id)); 
      setToast({ message: `${confirmDelete.name} deleted successfully!`, type: 'success' });
      close(); 
    } catch (e) { 
  ...
  const filteredStudentData = useMemo(() => {
    let d = studentData;
    if (statsFilter === 'Low Hours') {
      d = d.filter(s => s.status === 'Active' && isHoursCritical(s));
    }
    return d;
    ...
    <div style={{ minHeight: '100vh', fontFamily: "'Inter', -apple-system, sans-serif" }}>
      ...
      <div className="hdr" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20, gap: 12, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', justifyContent: 'flex-end' }}>
          <button onClick={() => navigate('/admin/students/reports')} title="Enrollment stats, red-flagged students, free days and attendance ranking" style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 18px', background: '#fff', color: '#4338CA', border: '1.5px solid #C7D2FE', borderRadius: 11, fontWeight: 700, fontSize: 13, cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}>
            <BarChart3 size={15} /> Reports
          </button>
          <button onClick={() => navigate('/admin/reports/flexi-hours')} title="Comprehensive flex report for all students" style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 18px', background: '#F0FDFA', color: '#0F766E', border: '1.5px solid #99F6E4', borderRadius: 11, fontWeight: 700, fontSize: 13, cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0, transition: 'all .15s' }}
      ...
      <div className="stats-row" style={{ display: 'grid', gap: 12, marginBottom: 18 }}>
        {stats.map(({ label, filterKey, val, icon: Icon, color, bg, border }) => {
          const isActive = statsFilter === filterKey;
          return (
            <div 
              key={label} 
              onClick={() => setStatsFilter(isActive ? null : filterKey)}
              style={{ 
                background: isActive ? bg : '#fff', 
                borderRadius: 14, 
                padding: '14px 16px', 
                border: `1.5px solid ${isActive ? color : '#F1F5F9'}`, 
                boxShadow: isActive ? `0 4px 12px ${color}25` : '0 1px 6px rgba(0,0,0,0.04)', 
                display: 'flex', 
                alignItems: 'center', 
                gap: 12,
                cursor: 'pointer',
      ...
      <div className="filt" style={{ background: '#fff', borderRadius: 14, padding: '12px 16px', border: '1px solid #F1F5F9', display: 'flex', gap: 9, marginBottom: 18, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <select
          ...
          style={{ ...IS, width: 'auto', minWidth: 160, background: sessionFilter !== 'all' ? '#EEF2FF' : '#F8FAFC', borderColor: sessionFilter !== 'all' ? '#C7D2FE' : '#E2E8F0' }}
        >
          <option value="all">🗓️ All Sessions</option>
          {academicSessions.map(s => (
            <option key={s._id} value={s._id}>{s.name}{s.status === 'Active' ? ' (Active)' : s.status === 'Upcoming' ? ' (Upcoming)' : ''}</option>
          ))}
        </select>
            ...
            ? pageLoading
              ...
              : filteredStudentData.length === 0
                ...
                : <div className="stu-grid" style={{ display: 'grid', gap: 16 }}>
                    {filteredStudentData.map(s => <StudentCard key={s._id} student={s} onView={s => open('view', s)} onEdit={s => open('edit', s)} onDelete={handleDelete} onQR={s => open('qr', s)} onToggleStatus={handleStatusToggle} onFlexReport={s => navigate(`/admin/flex-report/${s._id}`)} classesData={classesData} />)}
                  </div>
            : pageLoading
              ? <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '64px 0', gap: 12, color: '#94A3B8' }}>
                  <div style={{ width: 36, height: 36, borderRadius: '50%', border: '3px solid #EFF6FF', borderTopColor: '#0F4C5C', animation: 'spin .8s linear infinite' }} />
