...
/**
 * Priority → theme colors (accent, soft background, badge)
 */
...
const humanizeValue = (key, value) => {
  ...
  // Try to detect date-like values
  ...
  if (typeof value === 'object') return null; // skip nested objects, keep it simple for teachers

  return String(value);
};

// Keys we never want to surface to a teacher — internal plumbing, not content
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-indigo-50 via-slate-50 to-slate-50">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-8 sm:pt-10 pb-16 sm:pb-20 relative">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-6">
            <div className="flex items-center gap-3 sm:gap-4">
              <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-white/15 backdrop-blur-sm flex items-center justify-center flex-shrink-0 border border-black/20">
                {unreadCount > 0 ? (
                  <BellRing className="w-6 h-6 sm:w-7 sm:h-7 text-black bell-ring" />
                ) : (
              ...
              <div>
                ...
                <p className="text-indigo-100 text-sm sm:text-base mt-0.5 flex items-center gap-1.5">
                  {unreadCount > 0 ? (
                    <>
            ...
            {unreadCount > 0 && (
              <button
                ...
                disabled={markingAll}
                className="inline-flex items-center justify-center gap-2 px-5 py-3 bg-white text-indigo-700 rounded-xl hover:bg-indigo-50 active:scale-[0.98] transition font-bold shadow-lg disabled:opacity-70 w-full sm:w-auto"
              >
                {markingAll ? (
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 -mt-10 sm:-mt-12 relative pb-16">
        {/* Filter pills */}
        <div className="bg-white rounded-2xl shadow-md border border-slate-100 p-2 flex gap-2 mb-6 overflow-x-auto">
          <button
            onClick={() => {
              setFilter('all');
            ...
            {unreadCount > 0 && (
              <span
                className={`text-xs font-extrabold px-1.5 py-0.5 rounded-full min-w-[20px] text-center ${
                  filter === 'unread' ? 'bg-white/25 text-white' : 'bg-indigo-100 text-indigo-700'
                }`}
              >
                {unreadCount}
              </span>
              ...
              return (
                <div key={notif._id}>
                  <div
                    ...
                    className="notif-card cursor-pointer rounded-2xl bg-white border border-slate-100 hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200 overflow-hidden"
                    style={{
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-3 mb-1">
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0 mt-1">
                            {!notif.isRead && (
                              <span className="w-2 h-2 rounded-full bg-indigo-600 pulse-dot" />
                            )}
                  ...
                  {isSelected && !notif.link && (
                    <div className="notif-card bg-white border border-slate-100 rounded-2xl p-5 sm:p-6 mt-2 mb-4 shadow-sm relative">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
        ...
        {displayedNotifications.length > 0 && pagination.page < pagination.pages && !loading && (
          <div className="mt-8 text-center">
            <button
              ...
              className="inline-flex items-center gap-2 px-6 py-3 bg-white border-2 border-indigo-100 text-indigo-600 rounded-xl hover:bg-indigo-50 hover:border-indigo-200 active:scale-[0.98] transition font-bold text-sm shadow-sm"
            >
              Load more
            </button>
          </div>
